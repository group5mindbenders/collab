...
//        console.log(tool)
  ...
  function erase(x0, y0, x1, y1, color, linewidth, emit){
    context.beginPath();
...
function onEraseEvent(data){
  var w = canvaso.width;
  var h = canvaso.height;
  erase(data.x0 * w, data.y0 * h, data.x1 * w, data.y1 * h, data.color, data.lineThickness);
}

socket.on('drawing', onEraseEvent);
...
if (tool.started) {
  erase(tool.x0, tool.y0, ev._x, ev._y, colorPicked, lineWidthPicked, true);
  tool.x0 = ev._x;
  tool.y0 = ev._y;
}
};
...
function save_state(canvaso, list, keep) {
  undo.push(canvaso.toDataURL());
}

document.querySelector("#download").addEventListener("click", function () {
  var a = document.createElement("a")
  a.href = canvaso.toDataURL();
  a.download = "img.png";
  a.click()
  console.log('a');
});
